...
However, we can apply linear projections to the `Query`, `Key`, and `Value` vectors
...
Here we implement an different compatibility functions in Pytorch. The whole implementation is based on the matrix form of the attention mechanism.
We assume the second-last of the tensors is the sequence length, and the last is the embedding dimension.
