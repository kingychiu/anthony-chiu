...
$$
\mathbf{o} \in \R^{d_v} = \sum_{s=1}^{S} a_s \mathbf{v}_s = \sum_{s=1}^{S} \frac{\exp(e_s)}{\sum_{j=1}^S \exp(e_j)} \mathbf{v}_s = \sum_{s=1}^{S} \frac{\exp(f_{\mathbf{compatibility}}(\mathbf{q}, \mathbf{k}_s))}{\sum_{j=1}^S \exp(f_{\mathbf{compatibility}}(\mathbf{q}, \mathbf{k}_j))} \mathbf{v}_s
$$
...
If the original `Query`, `Key`, `Value` vectors are not the same size.
