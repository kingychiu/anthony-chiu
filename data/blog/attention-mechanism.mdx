...
$$
\forall 1 \le s \le S: a_s \in \R = \frac{\exp(e_s)}{\sum_{j=1}^S \exp(e_j)}
$$

Then the `Output` vector $\mathbf{o} \in \R^{d_v}$ is the weighted sum of those $S$ `Value` vectors:

$$
\mathbf{o} \in \R^{d_v} = \sum_{s=1}^{S} a_s \mathbf{v}_s
$$
...
class AbstractAttention(torch.nn.Module):
  ...
  def forward(
    ...
    if attention_mask is not None:
      compatibility_scores = compatibility_scores.masked_fill(attention_mask == 0, float('-inf'))
