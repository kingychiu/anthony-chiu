...
- $P(X=x_i|Y=y_j)$ is the likelihood. it can be written as $L(Y=y_j|X=x_i)$.
  It is the probability of finding the evidence $(X=x_i)$ given the $(Y=y_j)$ has already occured.
...
In Bayesian Probability, $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)}$ measures the support of an evidence $(X=x_i)$ for $(Y=y_j)$.

- If $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} > 1$, $P(Y=y_j|X=x_i) > P(Y=y_j)$. This indicates it is more likely to find the evidence $(X=x_i)$ given $(Y=y_j)$. This shows the evidence $(X=x_i)$ is for $(Y=y_j)$.
- If $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} = 0$, $P(Y=y_j|X=x_i) = P(Y=y_j)$. This indicates $(X=x_i)$ and $(Y=y_j)$ are independent.
- If $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} < 1$, $P(Y=y_j|X=x_i) < P(Y=y_j)$. This indicates it is less likely to find the evidence $(X=x_i)$ given $(Y=y_j)$. There might be another outcomes of $X$ that is for $(Y=y_j)$.

With $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)}$, we can update our prior personal belief $P(Y=y_j)$.

**TODO**: Example for Bayesian update.

**TODO**: Dive deeper into Bayesian inference.
