...
Then each observation in the experiment is a pair $(X=x_i, Y=y_j)$. $P(X=x_i, Y=y_j)$ denotes the joint probability of the event $(X=x_i, Y=y_j)$. For example, if $X$ and $Y$ are the smartphone model and the gender we
...
After obtaining the joint probability of each event/pair, we can get the joint probability distribution $P(X, Y)$.
...
And apply this to all $X$'s outcomes we get the marginal probability distribution $P(X)$:
