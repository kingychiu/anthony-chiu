...
- If $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} > 1$, $P(Y=y_j|X=x_i) > P(Y=y_j)$. This shows the evidence $(X=x_i)$ is for $(Y=y_j)$.
- If $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} = 0$, $P(Y=y_j|X=x_i) = P(Y=y_j)$. This indicates $(X=x_i)$ and $(Y=y_j)$ are independent.
- If $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} < 1$, $P(Y=y_j|X=x_i) < P(Y=y_j)$. There might be another outcomes of $X$ that is for $(Y=y_j)$.
