...
- $(Y=y_j)$ is the event we want to analyze and $(X=x_i)$ is the event we takes as an evidence to support the occurance of $(Y=y_j)$.
...
- $P(X=x_i)$ is the prior probability of finding the evidence. It normalize the likelihood $P(X=x_i|Y=y_j)$.
