...
In-terms of probability, $P(Y=y_j|X=x_i)$ describes the probability of finding our target $(Y=y_j)$ given the evidence $(X=x_i)$.
...
Therefore, $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)}$ can be understood as the support of the evidence for our target, since $\frac{P(X=x_i|Y=y_j)}{P(X=x_i)} > 1 \rightarrow P(Y=y_j|X=x_i) > P(Y=y_j)$, . This shows the evidence $(X=x_i)$ is for $(Y=y_j)$.
...
Let's say we have observed a dataset $\bold{D}$. In that dataset there are $N$ samples. We want to estimate a model's parameter $\bold{w}$ that is the best fit to the
dataset $\bold{D}$ from all possible parameters $\bold{W}$. The likelihood $L(\bold{W}=\bold{w}|\bold{D})$ is the measure of how likely one can observe this dataset $\bold{D}$ given $\bold{W}=\bold{w}$ is the correct $P(\bold{D}|\bold{W}=\bold{w})$.

## Maximum Likelihood Estimation

There are $N$ samples $\{\bold{x_1},\bold{x_2}...\bold{x_N}\}$ in the dataset $\bold{D}$:

$$
P(\bold{D}|\bold{W}=\bold{w}) = P(\bold{x_1}, \bold{x_2} ... \bold{x_N}|\bold{W}=\bold{w})
$$

Assume those samples are independent:

$$
P(\bold{D}|\bold{W}=\bold{w}) = P(\bold{x_1}|\bold{W}=\bold{w}) \times P(\bold{x_2}|\bold{W}=\bold{w}) ... P(\bold{x_N}|\bold{W}=\bold{w})
= \prod_{i=1}^{N} P(\bold{x_i}|\bold{W}=\bold{w})
$$

To find the best $\bold{W}=\bold{w}$, we going to maximize the likelihood:

$$
\bold{\hat{w}} = \argmax_{\bold{w}\in\bold{W}}{P(\bold{D}|\bold{W}=\bold{w})} = \argmax_{\bold{w}\in\bold{W}}{\prod_{i=1}^{N} P(\bold{x_i}|\bold{W}=\bold{w})}
$$

The objective is also equivalent to maximize the log-likelihood:

$$
\bold{\hat{w}} = \argmax_{\bold{w}\in\bold{W}}{P(\bold{D}|\bold{W}=\bold{w})} = \argmax_{\bold{w}\in\bold{W}}\sum_{i=1}^{N}{\ln  P(\bold{x_i}|\bold{W}=\bold{w})}
$$
