...
Let's say we have observed a dataset $\bold{D}$. In that dataset there are $N$ samples. We want to estimate a model's parameter $(\bold{W}=\bold{w})$ that is best fit to the
dataset $\bold{D}$ from all possible parameters. The likelihood $L(\bold{W}=\bold{w}|\bold{D})$ is the measure of how likely one can observe this dataset $\bold{D}$ given $\bold{W}={w}$ is the correct $P(\bold{D}|\bold{W}=\bold{w})$.
...
$$
\bold{\hat{w}} = \argmax_{\bold{w}\in\bold{W}}{\prod_{i=1}^{N} P(\bold{x_i}|\bold{W}=\bold{w})} = \sum_{i=1}^{N}{\ln  P(\bold{x_i}|\bold{W}=\bold{w})}
$$
