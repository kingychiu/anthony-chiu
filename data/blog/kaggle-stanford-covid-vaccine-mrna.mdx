...
- Then $H_{2_{(s,c)}} = (A \times H_1)_{(s,c)} = \sum_{s=1}^{S} A_{(s,s)}H_{1_{(s,c)}}$
  ...
  - $A$ is the pairing probabilities matrix. An unpaired base with position $s$ has a zero or close to zero $A_{(s,s)}$. As a result, the s-th vector of $H_2​$ represents the information coming from the sequence positions paired with the s-th position.
