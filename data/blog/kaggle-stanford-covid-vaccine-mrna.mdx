...
We firstly need to add self-loops adjancency $\mathbf{I}  \in \mathbb{R}^{S \times S}$ to $\mathbf{A}$:

<div style={{overflowX: "auto"}}>
$$
\underset{S \times S}{\mathbf{\hat{A}}} = \underset{S \times S}{\mathbf{A}} + \underset{S \times S}{\mathbf{I}}
$$
</div>

Next we compute the degree matrix $\mathbf{\hat{D}} \in \mathbb{R}^{S \times S}$ of $\mathbf{\hat{A}}$,
...
<div style={{overflowX: "auto"}}>
$$
\underset{S \times C}{\mathbf{GraphConv}(\mathbf{X}, \mathbf{A})} = f_{activation}(\mathbf{\hat{D}^{-\frac{1}{2}}} \cdot \mathbf{\hat{A}} \cdot \mathbf{\hat{D}^{-\frac{1}{2}}} \cdot \mathbf{X} \cdot \mathbf{W} + \mathbf{b})
$$
</div>
...
- Let's consider $\mathbf{H} = \mathbf{X} \cdot \mathbf{W}$, with value: $H_{s, c} = \sum_{d=1}^{D} X_{s,d}W_{d,c}$

  - So, the model learns a weight $\mathbf{W}$ that filters / aggregates the original feature dimension $D$ into the channel dimension $C$.

- Next let's take a look at the term $\mathbf{\hat{D}^{-\frac{1}{2}}} \cdot \mathbf{\hat{A}} \cdot \mathbf{\hat{D}^{-\frac{1}{2}}}$

- Then let $\mathbf{J_{s,c}} = \mathbf{A} \cdot \mathbf{H}$, with value: $\sum_{s=1}^{S} A_{s,s}H_{s,c}$
  - The filtered / aggregated hidden states $H_1$ is then multiplied with the given adjacency matrix $A$.
  - Two unpaired bases with position $s_1$ and $s_2$ have a zero or close to zero $A_{(s_1,s_2)}$; therefore the information from $H_{1_{(s2, c)}}$ is blocked from propagating to $H_{2_{(s1, c)}}$.
  - Two paired bases with position $s_1$ and $s_2$ have a high $A_{(s_1,s_2)}$ value; therefore the information from $H_{1_{(s2, c)}}$ is propagating to $H_{2_{(s1, c)}}$.
