...
To compute the output of a `GraphConv Layer` with $c$ channels applying on a sequence $\mathbf{X} \in \Re^{s \times d}$ and a adjacency matrix $\mathbf{A} \in \Re^{s \times s}$, we firstly need to compute
$\mathbf{\hat{A}} \in \Re^{s \times s}$:
...
<div style={{overflowX: "auto"}}>
$$
\underset{s \times c}{\mathbf{GraphConv}(\mathbf{X}, \mathbf{A})} = \mathbf{\hat{D}^{-\frac{1}{2}}} \cdot \mathbf{\hat{A}} \cdot \mathbf{\hat{D}^{-\frac{1}{2}}} \cdot \mathbf{X} \cdot \mathbf{W} + \mathbf{b}
$$
</div>

Where $\mathbf{W}$ and $\mathbf{b}$ are the learning weights, and $s$ and $d$ denotes the sequence length and the feature dimension respectively.
